...
1. **抽象メソッドを文書化**: サブクラスがどのメソッドを実装する必要があるかを明確に文書化してください。

2. **可能な場合はデフォルト実装を提供**: 合理的なデフォルトがあるメソッドには具象実装を提供してください。

3. **意味のあるエラーメッセージを使用**: NotImplementedErrorを発生させる際は、明確なガイダンスを提供してください。

4. **すべての抽象メソッドに型を付ける**: エラーを発生させる場合でも、抽象メソッドには完全な型アノテーションを付けるべきです。

5. **抽象クラスをフォーカスされた状態に保つ**: 抽象クラスは明確なコントラクトを定義すべきで、多くのことをしようとしないでください。

6. **具象実装でテスト**: 抽象クラスの設計を検証するために、常に少なくとも1つの具象実装を作成してください。
...
T-Rubyの抽象クラスは以下を提供します：

- **テンプレート定義**: サブクラスが従うべきテンプレート
- **部分実装**: 具象ヘルパーメソッドを含む
- **型安全なコントラクト**: サブクラスが必要なメソッドを実装することを保証
- **ポリモーフィズム**: サブクラスを相互に置き換え可能

関連するクラスファミリーが共通の動作を共有するが、特定の実装で異なる場合に抽象クラスを使用してください。テンプレートメソッド、ストラテジー、プラグインアーキテクチャに最適です。
